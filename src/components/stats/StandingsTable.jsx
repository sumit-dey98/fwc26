...
const sticky = 'sticky z-10 bg-navy-900 group-hover:bg-navy-700 transition-colors'
      ...
      <table className="w-full text-xs min-w-[560px]" style={{ tableLayout: 'fixed' }}>
        ...
        <thead>
          <tr className="border-b border-navy-700">
            ...
            <th className="text-left px-2 py-2 font-label font-semibold uppercase tracking-widest text-content-muted">
              Team
            </th>
            {COLS.map(c => (
              <th key={c} className={cn(
                'py-2 text-center font-label font-semibold uppercase tracking-widest',
                c === 'Pts' ? 'text-gold-500' : 'text-content-muted'
              )}>
                {c}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={row.team} className={cn(
              'border-b border-navy-800 group',
              i < 3 && 'border-l-2 border-l-green-600'
            )}>
              <td className={cn('px-4 py-3 text-content-muted tabular left-0', sticky)}>
                {i + 1}
              </td>
